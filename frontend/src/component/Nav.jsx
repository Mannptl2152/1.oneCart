...
function Nav() {
  ...
  const handleLogout = async () => {
    setLoggingOut(true);
    try {
      const res = await axios.get(`${base}/api/auth/logout`, {
        withCredentials: true, // required if backend uses cookie-based auth
      });
      console.log("logout response:", res.data);

      // After successful logout, refresh user state by calling the function
      // make sure getCurrentUser updates the userData to null/guest
      if (typeof getCurrentUser === "function") {
        await getCurrentUser(); // <-- important: call it
      }
  ...
  return (
    <>
      <div className="w-[100vw] h-[70px] bg-[#ecfafaec] z-10 fixed top-0 flex items-center justify-between px-[30px] shadow-md shadow-black">
        ...
        <div className="w-[30%] flex items-center justify-end gap-[20px] relative">
          ...
          <div className="relative">
            ...
            />
            <p className="absolute w-[18px] h-[18px] flex items-center justify-center bg-black px-[5px] py-[2px] text-white rounded-full text-[9px] top-[-8px] right-[-10px] hidden md:flex">
      ...
      {showSearch && (
        <div className="w-[100%] h-[80px] bg-[#d8f6f9dd] absolute top-[70px] left-0 right-0 flex items-center justify-center">
          <input
            type="text"
      ...
      {showProfile && (
        <div className="absolute w-[220px] h-[150px] bg-[#000000d7] top-[110px] right-[4%] border-[1px] border-[#aaa9a9] rounded-[10px] z-10">
